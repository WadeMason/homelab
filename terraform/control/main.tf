...
variable "hostname" { default = "kube-control" }
...
data "ignition_config" "kubernetes" {
  files = [
    data.ignition_file.file_hostname.rendered,
...
resource "libvirt_ignition" "ignition" {
  name    = "${var.hostname}-ignition"
  content = data.ignition_config.kubernetes.rendered
}
...
resource "libvirt_domain" "kube-control" {
  autostart = true
  name      = var.hostname
